<?php
  ...
  class User implements Crud, Authenticator
  {
    ...
    public function save($con, $target_file)
    {
      ...
      $this->hashPassword();
      $pass = $this->password;
      $res = mysqli_query($con, "INSERT INTO user(first_name, last_name, user_city, username, pass_word) VALUES('$fn', '$ln', '$city', '$uname', '$pass')") or die("Error: ". mysqli_error($con));
    ...
    public static function isPasswordCorrect($username, $password)
    {
      ...
      while ($row =  $res->fetch_assoc()) {
        if (password_verify($password, $row['pass_word']) && $username == $row['username']) {
          $found = true;
        }
      }
